...
A report consists of three areas, which you edit in the *Design* tab:
...
- **Body** — contains one or more data tables. Add columns via the **+** icon or by drag & drop from the field list; adjust width and order by drag & drop as well.
...
- **Merged fields:** In the column editor you can combine several fields into a single column; the separator (comma, space, line break) is configurable.
...
- The SUM label in the footer can be shown or hidden; the footer itself is enabled in the column editor's *Footer* tab.

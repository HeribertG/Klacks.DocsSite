...
Une règle de planification est rattachée à un **contrat**. Les valeurs limites définies s'appliquent ainsi à tous les collaborateurs ayant ce contrat. Chaque règle comprend trois domaines :
...
- Lors de la configuration initiale, Klacks propose un modèle pour ces valeurs adapté au pays et au secteur d'activité (jours fériés, majorations, règles de planification) ; ce modèle est un point de départ — il n'est pas mis à jour automatiquement au fil des changements législatifs et reste entièrement modifiable par l'administrateur.

---
sidebar_position: 2
---

# Ketidakhadiran dalam Pandangan

Cuti, sakit, latihan lanjutan, tentera — siapa yang tiada bila, dapat dilihat dalam Klacks sebagai carta Gantt yang jelas sepanjang tahun.

## Cara Ia Berfungsi

- Setiap Ketidakhadiran muncul sebagai bar berwarna pada garis masa individu berkenaan — pertindihan dan penumpuan segera kelihatan jelas.
- **Jenis Ketidakhadiran tersendiri** ditakrifkan sendiri oleh anda, dengan warna dan kelakuan (dibayar/tidak dibayar, dengan/tanpa kesan jam).
- Eksport sebagai **PDF** berjaya dengan satu klik — praktikal untuk paparan atau fail.
- Ketidakhadiran memberi kesan langsung kepada perancangan: Sesiapa yang tiada tidak akan dijadualkan oleh [Auto-Perancangan: Satu Klik, Selebihnya Berjalan di Latar Belakang](../planen/auto-planung.md), dan dalam [Grid Jadual: Matriks Masa Interaktif Anda](../planen/plan-raster.md), konflik itu segera kelihatan.

## Perkara Berguna untuk Diketahui

- Gambaran Gantt sesuai digunakan untuk perancangan cuti dalam pasukan: kekangan sepanjang musim panas kelihatan sekali imbas.
- Jika seseorang tiada secara mendadak, Klacks segera menunjukkan penggantian yang layak dan tersedia — tanpa menjungkirbalikkan keseluruhan jadual.
- Nilai Ketidakhadiran (contohnya catatan satu hari cuti penuh atau separuh) disertakan sebagai jumlah dalam analisis jam laporan.

![Paparan garis masa dalam Klacks](/img/app-timeline-de.png)

---
*Cuba secara langsung: [Klacks Playground](https://klacks-software.ch:7643) — Log masuk `admin@test.com` / `P@ssw0rt1`, data ditetapkan semula setiap hari.*
